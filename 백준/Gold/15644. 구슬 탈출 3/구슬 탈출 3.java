...
public class Main {
	...
	static class State {
		...
		public State(Marble color1, Marble color2, int time, ArrayList path) {
			this.color1 = color1;
			this.color2 = color2;
			this.time = time;
			this.path = path;
	...
	public static void main(String[] args) {
		...
		State init = new State(red, blue, 0, new ArrayList<>());
		queue.offer(init);
			...
			if (now.color1.c != redL.c || now.color2.c != blueL.c) {				
				if (blueL.r != -1 && redL.r != -1) {
					if (redL.r != blueL.r || redL.c != blueL.c) {
						ArrayList<String> temp = (ArrayList<String>) now.path.clone();
						temp.add("L");
						queue.offer(new State(redL, blueL, now.time+1, temp));
					}
				} else if (blueL.r != -1 && redL.r == -1) {
					System.out.println(now.time + 1);
					for (String s : now.path) {
						System.out.print(s);
					}
			...
			if (now.color1.c != redR.c || now.color2.c != blueR.c) {
				if (blueR.r != -1 && redR.r != -1) {
					if (redR.r != blueR.r || redR.c != blueR.c) {
						ArrayList<String> temp = (ArrayList<String>) now.path.clone();
						temp.add("R");
						queue.offer(new State(redR, blueR, now.time+1, temp));
					}
			...
			if (now.color1.r != redU.r || now.color2.r != blueU.r) {				
				if (blueU.r != -1 && redU.r != -1) {
					if (redU.r != blueU.r || redU.c != blueU.c) {
						ArrayList<String> temp = (ArrayList<String>) now.path.clone();
						temp.add("U");
						queue.offer(new State(redU, blueU, now.time+1, temp));
					}
			...
			if (now.color1.r != redD.r || now.color2.r != blueD.r) {				
				if (blueD.r != -1 && redD.r != -1) {
					if (redD.r != blueD.r || redD.c != blueD.c) {
						ArrayList<String> temp = (ArrayList<String>) now.path.clone();
						temp.add("D");
						queue.offer(new State(redD, blueD, now.time+1, temp));
					}
